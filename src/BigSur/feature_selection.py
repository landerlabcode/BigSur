...
def do_loop_ks_calculation(batch_dict, gene_row):
    ...
    if len(batch_dict.keys()) > 1:
        subsetmat = np.hstack([batch_dict[batch]['e_mat'][:,gene_row] for batch in batch_dict if batch != 'All'])
        chi_vec = np.hstack([np.repeat(batch_dict[batch]['Chi'], batch_dict[batch]['e_mat'].shape[0]) for batch in batch_dict if batch != 'All'])
    else:
        subsetmat = batch_dict['All']['e_mat'][:,gene_row]
    ...
    # subsetmat2 = ne.evaluate("subsetmat**2", {'subsetmat': subsetmat})
    # subsetmat3 = ne.evaluate("subsetmat**3", {'subsetmat': subsetmat})
    # subsetmat4 = ne.evaluate("subsetmat**4", {'subsetmat': subsetmat})
    # subsetmat5 = ne.evaluate("subsetmat**5", {'subsetmat': subsetmat})
    # subsetmat6 = ne.evaluate("subsetmat**6", {'subsetmat': subsetmat})
    # subsetmat7 = ne.evaluate("subsetmat**7", {'subsetmat': subsetmat})

    # dict_for_vars = {
    #     'subsetmat': subsetmat,
    #     'subsetmat2': subsetmat2,
    #     'subsetmat3': subsetmat3,
    #     'subsetmat4': subsetmat4,
    #     'subsetmat5': subsetmat5,
    #     'subsetmat6': subsetmat6,
    #     'subsetmat7': subsetmat7,
    #     'chi': chi_vec,
    #     'n_cells': total_n_cells_in_dataset
    # }

    k2 = -(1/total_n_cells_in_dataset) + ne.evaluate('sum((1+subsetmat*(-4+7*chi+6*subsetmat*(1-2*chi+chi**3)+subsetmat2*(-3+6*chi-4*chi**3+chi**6)))/(subsetmat*(n_cells+subsetmat*n_cells*(-1+chi))**2))', dict_for_vars)
