    ...
    if previously_run:
        if os.path.isfile(write_out + 'cumulants.npz'):
            ...
            if verbose > 1:
                ...
                if time_diff < 60:
                    print(
                        f"Finished calculating cumulants for {g_counts.shape[0]} genes in {(time_diff):04f} seconds."
                    )
                else:
                    print(
                        f"Finished calculating cumulants for {g_counts.shape[0]} genes in {(time_diff/60):04f} minutes."
                    )
    else:
        tic = time.perf_counter()
        kappa2, kappa3, kappa4, kappa5 = calculate_mcPCCs_cumulants(residuals, e_moments, e_mat, cv)
